...
raw_trajectories = list(Trajectory._get_collection().find({'info.prediction.classified_model': 'id'}, {}))

if not os.path.exists('lengths_and_durations_id_cache.txt'):
    ...
    with open('lengths_and_durations_id_cache.txt', 'w') as file:
        file.write('\n'.join(str(length_duration[0])+','+str(length_duration[1]) for length_duration in lengths_durations))
else:
    with open('lengths_and_durations_id_cache.txt', 'r') as file:
        lengths_durations = [(int(line.strip().split(',')[0]), float(line.strip().split(',')[1]))  for line in file.readlines()]

already_trained_networks = list(ImmobilizedTrajectorySegmentator.objects(
    simulator_identifier=CustomDataSimulation.STRING_LABEL,
    trained=True,
    hyperparameters=ImmobilizedTrajectorySegmentator.selected_hyperparameters()
))
...
for index, length_duration in tqdm.tqdm(list(enumerate(lengths_durations))):
    ...
    if len(available_networks) == 0:
        classifier = ImmobilizedTrajectorySegmentator(length, duration, simulator=CustomDataSimulation)
