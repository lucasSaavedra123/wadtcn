...
QUANTIFY_RPT_VS_STEPFINDER = False
...
for trajectory in trajectories:
    ...
    alpha_rpt_breakpoints = break_point_detection_with_stepfinder(alpha_result, 3, with_rpt=True)
    d_rpt_breakpoints = break_point_detection_with_stepfinder(d_result, 3, with_rpt=True)
    final_rpt_breakpoints = merge_breakpoints_and_delete_spurious_of_different_data(alpha_rpt_breakpoints, d_rpt_breakpoints, 3)
    ...
    rpt_rmse, rpt_jcs = single_changepoint_error(gt_cp, np.array(final_rpt_breakpoints), threshold = 10)

    if QUANTIFY_RPT_VS_STEPFINDER:
        rpt_rmses.append(rpt_rmse)
        rmses.append(rmse)
        rpt_jcss.append(rpt_jcs)
        jcss.append(jcs)
    else:
        """
        ...
        fig.suptitle(f'RMSE={np.round(rmse,2)}, JCS={np.round(jcs,2)}, RMSE(RPT)={np.round(rpt_rmse,2)}, JCS(RPT)={np.round(rpt_jcs,2)}')
...
if QUANTIFY_RPT_VS_STEPFINDER:
    print('RPT')
    print(np.mean(rpt_rmses), np.std(rpt_rmses))
    print(np.mean(rpt_jcss), np.std(rpt_jcss))
    print('AUTO STEP FINDER')
    print(np.mean(rmses), np.std(rmses))
    print(np.mean(jcss), np.std(jcss))
