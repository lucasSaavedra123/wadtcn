...
for trajectory in tqdm.tqdm(filtered_trajectories):
    available_networks = [network for network in classification_trained_networks if network.trajectory_length == trajectory.length and (network.trajectory_time * 0.85 <= round(trajectory.duration, 2) <= network.trajectory_time * 1.15)]
