...
class SlidingWindowHurstExponentPredicter(PredictiveModel):
    ...
    def predict_list_of_trajectories(self, trajectories):
        predictions = []
        ...
        for trajectory in trajectories:            
            ...
            for i in range(0, trajectory.length):
                initial_index = i
                final_index = i + self.trajectory_length

                if final_index > trajectory.length:
                    break

                prediction.append(self.predict([trajectory.build_noisy_subtrajectory_from_range(initial_index, final_index)])[0][0])
    ...
    def transform_trajectories_to_input(self, trajectories):
        ...
        for index, trajectory in enumerate(trajectories):
            X[index, :, 0] = axis_adaptation_to_net(trajectory.get_x(), self.trajectory_length)
            X[index, :, 1] = axis_adaptation_to_net(trajectory.get_y(), self.trajectory_length)
    ...
    @property
    def type_name(self):
        return f'segment_hurst_exponent_{self.extra_parameters["model"]}'
    ...
    def plot_predicted_and_ground_truth_histogram(self, trajectories=None):
        ...
        plot_predicted_and_ground_truth_histogram(ground_truth, predicted, a_range=[[0.05,1.95],[0.05,1.95]])
