...
from TheoreticalModels.ScaledBrownianMotion import ScaledBrownianMotion
...
class WaveNetTCNTheoreticalModelClassifier(PredictiveModel):
    ...
    def transform_trajectories_to_output(self, trajectories):
        Y_as_vectors = np.empty((len(trajectories), self.number_of_categories))

        for index, trajectory in enumerate(trajectories):
            Y_as_vectors[index, :] = to_categorical(self.model_to_label(trajectory.model_category), num_classes=self.number_of_categories)
    ...
    def __str__(self):
        return f"with_attetion_first_theoretical_model_classifier_length_{self.trajectory_length}"
